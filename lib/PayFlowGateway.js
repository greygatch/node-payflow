...
function PayFlowGateway (options) {
  ...
  this.host = options.testMode === true ? 'pilot-payflowpro.paypal.com' : 'payflowpro.paypal.com';
...
function lengthStringify(params)
{
  let qString = [];
  for(let param in params)
  {
    qString.push(param+'['+param[params[param].length]+']='+params[param]);
  }
  return qString.join('&');
}
...
function postRequest (params, service) {
  ...
  var body = lengthStringify(params);
...
PayFlowGateway.prototype.refundTransaction = function refundTransaction (transactionId, options) {
  ...
  if (options.amount) {
    params.AMT = (options.amount * 1).toFixed(2)
  }
